...
void extrcBoundary_realtime(vector<vector<double>> vectorBoundary,vector<vector<double>> &boudary_merge)
{
        vector<double> boudary_z;
        vector<double> boudary_x;    
                for (size_t i = 0; i < 20; i++)
                {
                        vector<double>::iterator it = find(vectorBoundary[i].begin(), vectorBoundary[i].end(), 1);
